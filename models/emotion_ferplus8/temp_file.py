...
zonnx.onnx(model).inputs('testferemotions.npy').update()

# Load onnx file and run inference
session = rt.InferenceSession('emotion-ferplus-8.onnx')
...
filename = os.path.join(dir, 'emotion_ferplus8')
if not os.path.exists(filename + '/ztn'): os.makedirs(filename + '/ztn')
zcontext.save(filename + '/ztn/emotion_ferplus8.ztn')
